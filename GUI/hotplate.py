...
class HotplateGUI:
    def __init__(self, master):
        self.master = master;
        self.master.title("Hot Plate Controller")
    ...
    def _build_widgets(self):
        ...
        # Status display (Temp / SP / State)
        status_frame = ttk.LabelFrame(main, text="Hot Plate Status", padding=8)
        status_frame.grid(row=1, column=0, sticky="ew", pady=(8, 0))

        status_frame.columnconfigure(1, weight=1)

        ttk.Label(status_frame, text="Temp:").grid(row=0, column=0, sticky="w")
        ...
        self.sp_slider_label_var = tk.StringVar(value="SP to set: -- °C")
        ttk.Label(control_frame, textvariable=self.sp_slider_label_var).grid(
            row=0, column=0, columnspan=4, sticky="w"
        )

        # Slider from 20°C to 70°C
        self.sp_slider_var = tk.IntVar(value=25)

        # Left endpoint label "20"
        ttk.Label(control_frame, text="20").grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(control_frame, text="70").grid(row=1, column=2, sticky="e")
        ...
        plot_frame = ttk.LabelFrame(main, text="Temp / SP History", padding=8)
        ...
        self.temp_line, = self.ax.plot([], [], label="Temp", color="tab:blue")
    ...
    def update_slider_label(self, value):
        ...
        try:
            ...
            self.sp_slider_label_var.set("SP to set: -- °C")
            return
        self.sp_slider_label_var.set(f"SP to set: {v:.1f} °C")
    ...
    def on_set_sp(self):
        if self.client is None:
            messagebox.showwarning("Not connected", "Connect to the hot plate first.")
            return
    ...
    def _update_plot(self):
        ...
        # Y axis fixed 20–70 as requested
        self.ax.set_ylim(20, 70)
    ...
    def _handle_device_line(self, line: str):
        ...
        try:
            # Example: "Temp: 23.45 C | SP: 50.0 C | STATE: 1"
            ...
            # Temp
            temp_tokens = temp_part.split()  # ["Temp:", "23.45", "C"]
            temp_val = float(temp_tokens[1])
            self.temp_var.set(f"{temp_val:.2f} C")

            # SP
            sp_tokens = sp_part.split()  # ["SP:", "50.0", "C"]
            sp_val = float(sp_tokens[1])
            self.sp_var.set(f"{sp_val:.1f} C")
