...
#define ENC_B_PORT   GPIOE
#define ENC_B_PIN    GPIO_PIN_5

// PE4 -> EXTI4_IRQn
// PE5 -> EXTI9_5_IRQn
#define ENC_A_IRQn   EXTI4_IRQn
#define ENC_B_IRQn   EXTI9_5_IRQn

// Counts per mechanical revolution (quadrature counts)
#define ENCODER_CPR  240.0f
...
// PID variables - MUST be double type!
PID_TypeDef TPID;
uint16_t knobReading;
double setpoint = 0.0;      // Changed from float to double
double current_rpm = 0.0;    // Changed from float to double
double rpm[AVG_WINDOW];
double pid_output = 0.0;     // Changed from float to double
uint32_t pwm_out;
...
double Ki = 10.0;
double Kd = 0.02;
double Kt = 2.5; // custom parameter to account for rpm adjusting with duty cycle

// Encoder states
static volatile uint32_t enc_count = 0;
static volatile uint32_t enc_count_fetched = 0;


const uint32_t MIN_READ_MS = 50;			  // 10ms minimum to record steps
static volatile uint32_t last_read_ms = 0;
static volatile uint32_t dt_ms = 0;
